...
5. Optionally add the Worker URL to the GitHub repository variable `VITE_API_URL` under **Settings > Secrets and variables > Actions > Variables**. The Pages build defaults to `https://bcaipro-api.dinofreud.workers.dev`.
6. Push a commit to `main` to rebuild the Pages site.

Verify the Worker at `https://bcaipro-api.dinofreud.workers.dev/health` before testing the frontend. It should return `{ "ok": true }`.