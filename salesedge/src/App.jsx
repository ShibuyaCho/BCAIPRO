...
async function generateAllAngles(vehicleInfo) {
  return Promise.all(
    ANGLES.map(angle =>
      generateSingleImage(
        `Professional automotive dealership photo of ${vehicleInfo}. ${angle.prompt}. Clean white or gradient background, showroom lighting, high resolution, no text, no watermarks.`
      )
    )
  );
}
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: "text", text: `Describe this vehicle precisely for image generation: color, make, model, body style, trim level, any notable features. Be specific and concise. Context: ${vehicleInfo}` }
        ]
      }]
    })
  });
  const visionData = await visionRes.json();
  if (!visionRes.ok) throw new Error(visionData.error?.message || "Vision analysis failed");
  const visualDescription = visionData.choices?.[0]?.message?.content || vehicleInfo;

  return Promise.all(
    ANGLES.map(angle =>
      generateSingleImage(
        `Professional dealership-quality automotive photo of ${visualDescription}. ${angle.prompt}. Clean white or gradient background, showroom lighting, high resolution, no text, no watermarks.`
      )
    )
  );
}
            ...
            {["description", "images"].map((step, i) => {
              ...
              return (
                <div key={step} style={{
                  ...
                  {done ? "✓ " : active ? "· " : ""}{step === "description" ? "Writing description" : "Generating 4 angles"}
                </div>
              );
            })}
          </div>
